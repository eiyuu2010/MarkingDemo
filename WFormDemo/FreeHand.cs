...
namespace WFormDemo
{
    class FreeHand : Shape
    {
        ...
        public override void Delete(Point point, float threshold)
        {
            for (int i = 0; i < this.Points.Count; i++)
            {
                //Finds if a point is within a certain distance of the point to remove.

                if ((Math.Abs(point.X - this.Points[i].X) < threshold) &&
                    (Math.Abs(point.Y - this.Points[i].Y) < threshold))
                {
                    ...
                    //for (int n = i; n < this.Points.Count; n++)
                    //{
                    //    this.Points[n].ShapeNumber += 1;
                    //}
